execute as @e[type=item, nbt={Item: {id: "minecraft:iron_ingot", Count: 2b, tag: {Tags: ["steel"]}}}] at @s if entity @e[type=armor_stand, tag=gun_station, dx=0, dy=0, dz=0] run tag @s add blast_furnace_craft_steel
execute at @e[tag=blast_furnace_craft_steel] as @e[type=item, distance=..0.5, nbt={Item: {id: "minecraft:iron_ingot", Count: 4b, tag: {Tags: ["black_steel"]}}}] run tag @s add blast_furnace_craft_black_steel
...
tag @e[tag=blast_furnace_craft_steel] add blast_furnace_craft
