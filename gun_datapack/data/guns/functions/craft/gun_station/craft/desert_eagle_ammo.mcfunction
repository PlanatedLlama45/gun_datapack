execute as @e[type=item, nbt={Item: {id: "minecraft:iron_ingot", Count: 1b}}, nbt=!{Item: {tag: {Tags: ["steel"]}}}] at @s if entity @e[type=armor_stand, tag=gun_station, dx=0, dy=0, dz=0] run tag @s add desert_eagle_ammo_craft_ingot
