...
execute as @e[tag=gun_station_build_steel] at @s align xyz if block ~ ~-1 ~ smithing_table run tag @e[type=item, distance=..0.5, nbt={Item: {id: "minecraft:iron_ingot", Count: 1b, tag: {Tags: ["black_steel"]}}}] add gun_station_build_black_steel
